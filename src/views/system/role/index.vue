<template>
  <BasicTable @register="registerTable">
    <template #tableTitle>
      <a-button type="primary" preIcon="ant-design:plus-outlined" @click="handleCreate" v-if="isAdmin"> 新增</a-button>
      <a-button type="primary" preIcon="ant-design:export-outlined" @click="onExportXls"> 导出</a-button>
      <j-upload-button type="primary" preIcon="ant-design:import-outlined" @click="onImportXls" v-if="isAdmin">导入</j-upload-button>
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <template #overlay>
          <a-menu>
            <a-menu-item key="1" @click="batchHandleDelete" v-if="isAdmin">
...
<script lang="ts" name="system-role" setup>
  ...
  import { list, deleteRole, batchDeleteRole, getExportUrl, getImportUrl, getUserRoles } from './role.api';
  import { useListPage } from '/@/hooks/system/useListPage';
  
  const showFooter = ref(true);
  const isAdmin = ref(false); // 是否为管理员角色
  ...
  const { prefixCls, tableContext, onImportXls, onExportXls } = useListPage({
    ...
    tableProps: {
      ...
      afterFetch: (data) => {
        // 如果不是管理员，过滤掉角色名为"管理员"的数据
        if (!isAdmin.value) {
          return data.filter(item => item.roleName !== '管理员');
        }
        return data;
      },
    },
