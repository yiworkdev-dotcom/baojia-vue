...
enum Api {
  ...
  getByUserSubordinate='/dw/partner/dwPtPartner/mySubordinate',
}
export const getByUserSubordinate = (params) => defHttp.get({ url: Api.getByUserSubordinate, params });
/**
 * 导出api
 * @param params
 */
 ...
 * @param params
 */
export const listNoCareTenant = (params) => {
  const userStore = useUserStore();

  return defHttp.get({ url: Api.listNoCareTenant, params }).then((res) => {
    // 不是 admin 的情况下做过滤
    if (userStore.getUserInfo.username !== 'admin') {
      if (Array.isArray(res?.records)) {
        const originalLength = res.records.length;
        res.records = res.records.filter(
          (item) => item.username !== 'admin'
        );
        // 更新总数，减去被过滤掉的管理员账号数量
        const filteredLength = res.records.length;
        const adminCount = originalLength - filteredLength;
        if (res.total && adminCount > 0) {
          res.total = res.total - adminCount;
        }
      }
    }
    return res;
 ...
 * @param params
 */
export const getAllRolesListNoByTenant = (params) => {
  const userStore = useUserStore();
  return defHttp.get({ url: Api.allRolesListNoByTenant, params }).then((res) => {
    if (userStore.getUserInfo.username !== 'admin') {
        res = res.filter(
          (item) => item.roleName === '报价超级管理员' || item.roleName === '普通用户' || item.roleName === '报价一般管理员' || item.roleName === '报价客服'
        );
    }
    return res;
  });
