...
<script lang="ts" setup>
   ...
   /*   if (data.record.relTenantIds && !Array.isArray(data.record.relTenantIds)) {
      ...
      try {
        ...
        if (userRoles && userRoles.length > 0) {
          // 单选模式：取第一个角色的 id
          // 如果 userRoles 是对象数组，取第一个对象的 id；如果是 id 数组，取第一个 id
          if (Array.isArray(userRoles)) {
            const firstRole = userRoles[0];
            data.record.selectedroles = typeof firstRole === 'object' && firstRole.id ? firstRole.id : firstRole;
          } else {
  ...
  async function handleSubmit() {
    try {
      ...
      // -update-end--author:liaozhiyang---date:20240702---for：【TV360X-1737】部门用户编辑接口，增加参数updateFromPage:"deptUsers"
      // 将角色值转换为字符串格式（后端期望字符串格式）
      if (params.selectedroles !== undefined && params.selectedroles !== null && params.selectedroles !== '') {
        if (Array.isArray(params.selectedroles)) {
          // 如果是数组，取第一个元素
          params.selectedroles = params.selectedroles[0] || '';
        } else {
          // 已经是字符串，直接使用
          params.selectedroles = params.selectedroles.toString();
        }
      } else {
        // 如果没有选择角色，设置为空字符串
